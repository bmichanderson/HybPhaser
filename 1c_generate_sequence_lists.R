...
if (name_for_dataset_optimization_subset != "") {
	folder_subset_add <- paste0("_", name_for_dataset_optimization_subset)
} else {
	folder_subset_add <- ""
}

output_Robjects <- file.path(path_to_output_folder, "00_R_objects", name_for_dataset_optimization_subset)
output_sequences <- file.path(path_to_output_folder, paste0("03_sequence_lists", folder_subset_add))
...
# delete directory, if it existed in order to prevent errors
unlink(c(folder4seq_consensus_loci, folder4seq_consensus_samples, folder4seq_contig_loci,
	folder4seq_contig_samples), recursive = TRUE)
...
# if you are on Linux, then the bash commands cat and sed are used
# if not, then R file operations are used, which are slower

if (Sys.info()['sysname'] == "Linux") {
	if (intronerated_contig == "yes") {
		for (locus in targets_name) {
			command_cat_consensus <- paste("cat", file.path(path_to_output_folder, "01_data/*/intronerated_consensus/",
				paste0(locus, "_intronerated.fasta")), ">", file.path(folder4seq_consensus_loci, paste0(locus,
				"_intronerated_consensus.fasta")))
			command_cat_contig <- paste("cat", file.path(path_to_output_folder, "01_data/*/intronerated_contigs/",
				paste0(locus, "_intronerated.fasta")), ">", file.path(folder4seq_contig_loci, paste0(locus,
				"_intronerated_contig.fasta")))
			system(command_cat_consensus, ignore.stderr = TRUE)
			system(command_cat_contig, ignore.stderr = TRUE)
			command_remove_locus_in_seqnames_consensus <- paste0("sed -i 's/-", locus, "//g' ",
				file.path(folder4seq_consensus_loci, paste0(locus, "_intronerated_consensus.fasta")))
			system(command_remove_locus_in_seqnames_consensus)
			command_remove_locus_in_seqnames_contig <- paste0("sed -i 's/-", locus, "//g' ",
				file.path(folder4seq_contig_loci, paste0(locus, "_intronerated_contig.fasta")))
			system(command_remove_locus_in_seqnames_contig)
		}
	} else {
		for (locus in targets_name) {
			command_cat_consensus <- paste("cat", file.path(path_to_output_folder, "01_data/*/consensus/",
				paste0(locus, ".fasta")), ">", file.path(folder4seq_consensus_loci, paste0(locus,
				"_consensus.fasta")))
			command_cat_contig <- paste("cat", file.path(path_to_output_folder, "01_data/*/contigs/",
				paste0(locus, ".fasta")), ">", file.path(folder4seq_contig_loci, paste0(locus,
				"_contig.fasta")))
			system(command_cat_consensus, ignore.stderr = TRUE)
			system(command_cat_contig, ignore.stderr = TRUE)
			command_remove_locus_in_seqnames_consensus <- paste0("sed -i 's/-", locus, "//g' ",
				file.path(folder4seq_consensus_loci, paste0(locus, "_consensus.fasta")))
			command_remove_locus_in_seqnames_contig <- paste0("sed -i 's/-", locus, "//g' ",
				file.path(folder4seq_contig_loci, paste0(locus, "_contig.fasta")))
			system(command_remove_locus_in_seqnames_consensus)
			system(command_remove_locus_in_seqnames_contig)
		}
	}
	...
	if (intronerated_contig == "yes") {
		for (locus in targets_name) {
			...
			fasta_files <- list.files(path = file.path(path_to_output_folder, "01_data/"),
				pattern = paste0("^", locus, "_intronerated.fasta"), recursive = TRUE, full.names = TRUE)
			...
			#define output files
			output_file_consensus <- file.path(folder4seq_consensus_loci, paste0(locus,
				"_intronerated_consensus.fasta"))
			output_file_contigs <- file.path(folder4seq_contig_loci, paste0(locus,
				"_intronerated_contigs.fasta"))
			...
			lines_consensus <- gsub(paste0("-", locus), "", readLines(output_file_consensus))
			lines_contigs <- gsub(paste0("-", locus), "", readLines(output_file_contigs))
		...
		for (locus in targets_name) {
			#list all fasta files from that locus for all samples
			fasta_files <- list.files(path = file.path(path_to_output_folder, "01_data/"),
				pattern = paste0("^", locus, ".fasta"), recursive = TRUE, full.names = TRUE)
			#select consensus/contig files
			consensus_files <- grep("consensus", fasta_files, value = TRUE)
			contigs_files <- grep("contigs", fasta_files, value = TRUE)
			#define output files
			output_file_consensus <- file.path(folder4seq_consensus_loci, paste0(locus, "_consensus.fasta"))
			output_file_contigs <- file.path(folder4seq_contig_loci, paste0(locus, "_contigs.fasta"))
			#generate output files
			file.create(output_file_consensus, overwrite = TRUE)
			file.create(output_file_contigs, overwrite = TRUE)
			#append sample fastas to the empty output file
...
for (file in list.files(folder4seq_contig_loci, full.names = TRUE)) {
	file <- list.files(folder4seq_contig_loci, full.names = TRUE)[3]
...
loci_files_consensus <- list.files(path = folder4seq_consensus_loci, full.names = TRUE)
loci_files_contig <- list.files(path = folder4seq_contig_loci, full.names = TRUE)

loci_to_remove <- c(names(failed_loci), outloci_missing, outloci_para_all)

if (length(loci_to_remove) != 0) {
	if (intronerated_contig == "no") {
		loci_files_to_remove_consensus <- loci_files_consensus[which(gsub(".*/(.*)_consensus.fasta",
			"\\1", loci_files_consensus) %in% loci_to_remove)]
		loci_files_to_remove_contig <- loci_files_contig[which(gsub(".*/(.*)_contig.fasta",
			"\\1", loci_files_contig) %in% loci_to_remove)]
	} else {
		loci_files_to_remove_consensus <- loci_files_consensus[which(gsub(".*/(.*)_intronerated_consensus.fasta",
			"\\1", loci_files_consensus) %in% loci_to_remove)]
		loci_files_to_remove_contig <- loci_files_contig[which(gsub(".*/(.*)_intronerated_contig.fasta",
			"\\1", loci_files_contig) %in% loci_to_remove)]
	}
...
for (locus in rownames(tab_snps_cl2b)) {
	if (!(locus %in% names(failed_loci))) {
		...
		if (length(grep(paste0("\\b", locus, "\\b"), outloci_para_each)) > 0) {
			samples_to_remove <- c(samples_to_remove_4all,
				names(outloci_para_each[grep(paste0("\\b", locus, "\\b"), outloci_para_each)]))
		} else {
		...
		if (length(samples_to_remove) != 0) {
			# consensus
			locus_consensus <- readLines(file.path(folder4seq_consensus_loci,
				paste0(locus, "_", intronerated_name, intronerated_underscore, "consensus.fasta")))
			lines_with_samplename <- which(gsub(">", "", locus_consensus) %in% samples_to_remove)
			if (length(lines_with_samplename) != 0) {
				lines_to_remove <- c(lines_with_samplename, lines_with_samplename + 1)
				locus_file_red <- locus_consensus[-lines_to_remove]
				conn <- file(file.path(folder4seq_consensus_loci,
					paste0(locus, "_", intronerated_name, intronerated_underscore, "consensus.fasta")))
				writeLines(locus_file_red, conn)
				close(conn)
			}
			...
			# contig
			locus_contig <- readLines(file.path(folder4seq_contig_loci,
				paste0(locus, "_", intronerated_name, intronerated_underscore, "contig.fasta")))
			lines_with_samplename <- which(gsub(">", "", locus_contig) %in% samples_to_remove)
			if (length(lines_with_samplename) != 0) {
				lines_to_remove <- c(lines_with_samplename, lines_with_samplename + 1)
				locus_file_hp_red <- locus_contig[-lines_to_remove]
				conn <- file(file.path(folder4seq_contig_loci,
					paste0(locus, "_", intronerated_name, intronerated_underscore, "contig.fasta")))
				writeLines(locus_file_hp_red, conn)
				close(conn)
			}
...
if (Sys.info()['sysname'] == "Linux") {
	if (intronerated_contig == "yes") {
		for (sample in samples) {
			command_cat_loci_consensus <- paste("cat", file.path(path_to_output_folder, "01_data/",
				sample, "/intronerated_consensus/*.fasta"), ">", file.path(folder4seq_consensus_samples,
				paste0(sample, "_intronerated_consensus.fasta")))
			command_cat_loci_contig <- paste("cat", file.path(path_to_output_folder, "01_data/",
				sample, "/intronerated_contigs/*.fasta"), ">", file.path(folder4seq_contig_samples,
		...
		for (sample in samples) {
			command_cat_loci_consensus <- paste("cat", file.path(path_to_output_folder, "01_data/",
				sample, "/consensus/*.fasta"), ">", file.path(folder4seq_consensus_samples,
				paste0(sample, "_consensus.fasta")))
			command_cat_loci_contig <- paste("cat", file.path(path_to_output_folder, "01_data/",
				sample, "/contigs/*.fasta"), ">", file.path(folder4seq_contig_samples,
	...
	if (intronerated_contig == "yes") {
		for (sample in samples) {
			...
			consensus_files_samples <- list.files(path = file.path(path_to_output_folder, "01_data/",
				sample, "/intronerated_consensus/"), pattern = "*.fasta", full.names = TRUE)
			contigs_files_samples <- list.files(path = file.path(path_to_output_folder, "01_data/",
				sample, "/intronerated_contigs/"), pattern = "*.fasta", full.names = TRUE)
			#define output files
			output_file_consensus_samples <- file.path(folder4seq_consensus_samples,
				paste0(sample, "_intronerated_consensus.fasta"))
			output_file_contigs_samples <- file.path(folder4seq_contig_samples,
				paste0(sample, "_intronerated_contigs.fasta"))
			#create output files
		...
		for (sample in samples) {
			#list all fasta files from the sample for all loci
			consensus_files_samples <- list.files(path = file.path(path_to_output_folder, "01_data/",
				sample, "/consensus/"), pattern = "*.fasta", full.names = TRUE)
			contigs_files_samples <- list.files(path = file.path(path_to_output_folder, "01_data/",
				sample, "/contigs/"), pattern = "*.fasta", full.names = TRUE)
			#define output files
			output_file_consensus_samples <- file.path(folder4seq_consensus_samples,
				paste0(sample, "_consensus.fasta"))
			output_file_contigs_samples <- file.path(folder4seq_contig_samples,
				paste0(sample, "_contigs.fasta"))
			#create output files
			file.create(output_file_consensus_samples, overwrite = TRUE)
			file.create(output_file_contigs_samples, overwrite = TRUE)
			#append fasta files to empty output file
...
# changing interleaved HybPiper files to non-interleaved fasta files
for (file in list.files(folder4seq_contig_samples, full.names = TRUE)) {
	lines <- readLines(file)
	conx <- file(file)
	writeLines(str_split(paste(gsub("(>.*)", ":\\1:", lines), collapse = ""), pattern = ":")[[1]][-1], conx)
	close(conx)
...
# remove loci (missing data, paralogs for all)
###################################################
...
sample_files_consensus <- list.files(path = folder4seq_consensus_samples, full.names = TRUE)
sample_files_contig <- list.files(path = folder4seq_contig_samples, full.names = TRUE)
...
if (length(outsamples_missing) == 0) {
	...
	samples_files_to_remove_consensus <- sample_files_consensus[which(gsub(".*/(.*)_consensus.fasta",
		"\\1", sample_files_consensus) %in% outsamples_missing)]
	samples_files_to_remove_contig <- sample_files_contig[which(gsub(".*/(.*)_contigs.fasta",
		"\\1", sample_files_contig) %in% outsamples_missing)]
...
for (sample in samples_in) {
	...
	if (length(loci_to_remove) != 0) {
		#consensus
		consensus_file2clean <- file.path(folder4seq_consensus_samples,
			paste0(sample, intronerated_underscore, intronerated_name, "_consensus.fasta"))
		...
		#contig
		contig_file2clean <- file.path(folder4seq_contig_samples,
			paste0(sample, intronerated_underscore, intronerated_name, "_contig.fasta"))
